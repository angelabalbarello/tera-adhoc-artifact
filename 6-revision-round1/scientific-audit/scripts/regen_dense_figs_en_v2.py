...
def nats_axis(ax, axis="x"):
    # v2: official entropy unit is BITS (Eq. 11, log2); ticks at 0 / 0.5 / 1
    ticks = [0, 0.5, 1.0]
    labels = ["0", "0.5", "1"]
    if axis == "x":
...
def fig_entropy_dist(df, out):
    ...
    dv["H_nats"] = dv.H_t / LN2  # v2: logs em nats -> bits
    ...
    for ax, (cfg, title) in zip(axes[:2], [("baseline_fixed", "(a) Baseline — phases blur"),
        ...
        ax.set_title(title)
        ax.set_xlabel(r"Entropy $H_t$ (bits)")
...
def fig_temporal_trace(df, out):
    ...
    # v2: criterio OBJETIVO pre-declarado — para cada regime, o episodio cujo
    # contraste de entropia pre/pos onset e o MEDIANO entre os criticos do
    # regime (t0 em janela central para legibilidade). Sem selecao favoravel.
    windows = {"abrupt": (2.0, 4.0), "progressive": (4.0, 7.0)}
    for regime, (lo, hi) in windows.items():
        scores = {}
        for ep_id, g in base[base.regime == regime].groupby("episode_id"):
            g = g.sort_values("t")
            ...
            if len(pre) < 3:
                continue
            scores[ep_id] = float(g[g.time_s >= t0].H_t.mean()) - float(pre.H_t.mean())
        ordered = sorted(scores, key=scores.get)
        ep_ids[regime] = ordered[len(ordered) // 2]
        sel = dv[(dv.config == "afkd_hybrid") & (dv.episode_id == ep_ids[regime])]
        print(f"  [fig5-v2] {regime}: episode_id={ep_ids[regime]} "
              f"t0={float(sel.t0_time_s.iloc[0]):.2f}s "
              f"contrast_median={scores[ep_ids[regime]]:.3f} nats "
              f"(n_candidates={len(ordered)})")
    ...
    for row, (regime, rlabel) in enumerate([("progressive", "Progressive episode"),
        ...
        for col, (cfg, clabel, color) in enumerate(cfgs):
            ...
            ax2 = ax.twinx()
            ax2.plot(t, ep.H_t.values / LN2, color=color, lw=1.8, ls="--",
                     alpha=0.55, zorder=2)
            ax2.set_ylim(0, 1.04)
            if col == 1:
                ax2.set_ylabel(r"$H(t)$ (bits)")
            else:
                ax2.set_yticklabels([])
            t0 = float(ep.t0_time_s.iloc[0])
...
def fig_gating_heatmap(df, out):
    ...
    for ax, (cfg, title) in zip(axs, [("baseline_hybrid", "(a) Baseline+MHEG"),
        ...
        im = ax.imshow(mat, aspect="auto", cmap="YlOrRd",
                       interpolation="none", vmin=0, vmax=1.0)
    ...
    cb.set_label(r"Entropy $H_t$ (bits)", fontsize=12)
