...
def analyze_temporal_entropy(
    ...
    Fundamental para a demonstração da hipótese:
      · Baseline: H(t) uniforme — fases mal separadas
      · Calibrado: H(t) deslocado em escala mas mesmo perfil temporal
      · AF-TKD:    H(t) estratificado — fases claramente separadas
    """
...
def compute_entropy_stratification_score(profile: TemporalEntropyProfile) -> float:
    ...
    Score alto -> entropia discriminativa (como AF-TKD)
...
def fig_entropy_temporal_curves(
    ...
    enquanto AF-TKD apresenta elevação distinta ao redor de t0=0.
    """
    try:
    ...
    ax_cal.axvline(0, color="#cc0000", lw=0.9, ls=":", alpha=0.7,
                   label=r"$t_0$ (onset)")
    ax_cal.set_xlabel(r"Tempo relativo ao onset (s)")
    ax_cal.set_ylabel(r"$H(t)$ (bits)")
    ax_cal.set_title("(a) Calibração post-hoc vs. Baseline")
    ...
    ax_aftkd.axvline(0, color="#cc0000", lw=0.9, ls=":", alpha=0.7,
                     label=r"$t_0$ (onset)")
    ax_aftkd.set_xlabel(r"Tempo relativo ao onset (s)")
    ax_aftkd.set_title("(b) AF-TKD vs. Baseline (referência)")
    ...
    fig.suptitle(
        "Calibração post-hoc não reorganiza a dinâmica temporal da entropia",
...
def fig_ttdef_distributions(
    ...
    ax_hist.set_title("(a) Distribuição de frequência")
    ax_hist.axvline(T_MAX_EF, color="gray", lw=0.8, ls=":",
                    label=f"Penalidade máx. ({T_MAX_EF}s)")
    ax_hist.legend(fontsize=7, loc="upper right", framealpha=0.7)

    ax_cdf.set_xlabel(r"$\mathrm{TTD}_{\mathrm{ef}}$ (s)")
    ax_cdf.set_ylabel("Fração acumulada")
    ax_cdf.set_title("(b) CDF — cauda operacional")
    ...
    fig.suptitle(
        r"Calibração melhora ECE mas não elimina a cauda de $\mathrm{TTD}_{\mathrm{ef}}$",
        fontsize=9, y=1.01, style="italic"
    )
    fig.tight_layout()
    out_path.parent.mkdir(parents=True, exist_ok=True)
...
def fig_gating_heatmap_calibrated(
    ...
    Compara Baseline, Baseline+TS calibrado e AF-TKD.
    ...
    for ax, cid in zip(axes, [c for c in configs_show
        ...
        if valid_onsets:
            ax.axvline(np.mean(valid_onsets), color="#00ff88",
                       lw=1.0, ls="--", alpha=0.85, label="onset médio")

        ax.set_title(titles_show.get(cid, cid), fontsize=8)
        ax.set_xlabel("t (s)", fontsize=8)
        if ax == axes[0]:
            ax.set_ylabel("Episódios críticos", fontsize=8)
    ...
    fig.suptitle(
        r"$H(t)$ em episódios críticos: calibração não induz elevação ao redor do onset",
        fontsize=9, y=1.01, style="italic"
    )
    out_path.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(out_path)
...
def export_latex_table(
    ...
    lines = [
        ...
        "% ─────────────────────────────────────────────────────────────────────",
        "\\midrule",
        "\\multicolumn{9}{l}{\\textit{Calibração post-hoc sobre Baseline}} \\\\",
        "\\midrule",
    ]
...
class CalibrationExperiment:
    ...
    def _load_aftkd_frame_probs_test(self, seed: int) -> Optional[np.ndarray]:
        """Carrega frame_probs do AF-TKD no TEST (para referência nas figuras)."""
...
transição. Em contraste, o AF-TKD exibe elevação de $H(t)$
...
AF-TKD+MHEG ocupa o quadrante de baixo custo e baixo atraso.
