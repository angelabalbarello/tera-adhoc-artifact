...
## L. Rerun da calibração (24/09 — Figs. 12–13 e Tabela 14 FECHADOS)
A autora reexecutou `tera_posthoc_calibration_experiment.py` (PYTHONPATH=1-pipeline-tera, exp-dir=2-campaign): **96/96 macros de calibração reproduzidos identicamente** (diferença apenas de formatação decimal, vírgula vs. ponto), incluindo a linha IC+MHEG da Tabela 14 (ECE 0,0110; FR 0; TTDef 0,0527 s; 57,1% sup) e os H_strat (TS 0,7568 / PS 0,6763 / IC 0,8186). Fig. 12 (`fig_calibration_entropy_temporal.pdf`) regenerada com fontes ampliadas (patch R2.6 no script) e adotada no artigo; Fig. 13 permanece a versão do `regen_fig13_locus_en.py`. Na passada, três resíduos textuais do score H_strat removido foram encontrados e corrigidos (nota da Tab. 14, frase epistemológica da §4.8, parágrafo da Discussão) — todos agora apontam para a morfologia das trajetórias. Recompilado: 34 pp, 0 erros, verificador 0 FAIL/40 OK. Com isso, TODAS as figuras do artigo são regeneráveis a partir do repositório público (mapa em scientific-audit/FIGURE_PROVENANCE.md) e a resposta de reprodutibilidade é YES após o push pendente.
