...
The most informative result of this campaign is that, under its stress
protocol and at matched budget, structure rather than compute budget drives
near-onset safety; {\color{\revcolor}Section~\ref{sec:temporal_scale}
qualifies the scope of this reading under temporal-scale shift, where the
budget-matched advantage concentrates on coverage and timeliness rather than
on suppression placement.}
...
($33$--$46\%$ FS@onset), a result consistent with the kinematic trigger acting
as the intended fallback for sub-onset windows.
...
A per-channel ablation indicates that both components are necessary.
